...
function values=getvaluematch(targetString,instr)
	...
	%        targetString ='Upper Left  (  ';
        ...
        pattern = [regexptranslate('escape', targetString), '\s*([0-9.]+)\s*,\s*([0-9.]+)\s*\)'];

        % Use regexp on the character string
        matches = regexp(instr, pattern, 'tokens', 'once');

        % Check if the target string is found
        if ~isempty(matches)
            % Extract the two values following the match
