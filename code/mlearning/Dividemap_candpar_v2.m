...
%new area array
clear shpc;
shpc=struct2cell(S1)';areasv1=[shpc{:,6}]';
...
%for ix=1:ns
	...
	tic
	%Run small slumps using lots of CPUs.
	%Rerun large slumps ( > 2 tiles (2 minutes)) using less CPUs to save Computation resources and avoid idle and waiting.
	%Another thought: run 5 classes seperately.
	if (sz > 40 && areasv1(ix) < areathres) ||(sz <=40 && areasv1(ix) >=areathres)
...
if sz > 40 %small tiles, quick run
save test1_sml.mat -v7.3
else
save test1_large.mat -v7.3
end
...
if sz > 40 %small tiles, quick run
ofile=['ixy_',regionstr,'_cand_sml.mat'];
else
ofile=['ixy_',regionstr,'_cand_large.mat'];
end
