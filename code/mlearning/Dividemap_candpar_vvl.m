...
classtype='_vvl'; % _vs _s _m _l _vl _vvl
...
shapefile=['change2polyg_',regid,classtypein,'.shp';] %'../region09/v3con/change2polyg_09.shp'; %candidates change2polyg_07_vs.shp
mergedfile=[regiondir,'/region',regid,'_jump.tif'];

%if 1 %flagorthotif==1
        %save orthoimage in geotiff 
        %odirt1=['slumps_ortho',classtype,'/'];
        odirt1=['slumptif_',regionstr,classtype,'/'];
        if ~exist(odirt1,'')
            mkdir(odirt1)
        end
%end
odir1=strrep(odirt1,'slumptif_','slumpsub_');
% odirt2='slumps_dem/';
...
delete(poolobj)
   ...
   fprintf('Dividemap_candpar_vvl.m There was an error! The message was:\n%s',e.message);
end
...
ofile=['ixy_',regionstr,'_cand',classtype,'.mat'];
