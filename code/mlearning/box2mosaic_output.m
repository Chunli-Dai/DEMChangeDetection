function [datao]=box2mosaic_output(rangi,flagband,f,fdir,ix,regiondir,odir1)
...
%   	resr2m=2;
	...
	maxlen=max([abs(rangi(2)-rangi(1))*1e-3, abs(rangi(4)-rangi(3))*1e-3]);% maximum length in km
        if 1 %maxlen <= 1 % maximum side of the box is <=4 km; small tiles
               resr2m=2;
%              resr2m=10;
	...
	datao=data0;datao.z=nan(ny0,nx0,7); % jump jumpstd T1 T2 bitmask nov dem/curvature
	...
	matchingFiles =dir(file1); %dir(fullfile(directoryPath, fileNamePattern));
	if ~isempty(matchingFiles) %file found
		file1=fullfile(matchingFiles(1).folder,matchingFiles(1).name);
 	else
		file1b=strrep(file1,'/blue/chunlidai/chunlidai/landslide/','/orange/chunlidai/results/landslide/');
		matchingFilesb =dir(file1b);
		if ~isempty(matchingFilesb)
			file1=fullfile(matchingFilesb(1).folder,matchingFilesb(1).name);
		else
			fprintf(['\n No files found ',file1,'. \n'])
		end
	end
...
%	    data0.z(M)=tz(M);
	...
	%     return
	    ...
	    tzpre=data0T2.z;
% 	    M=isnan(tzpre)&~isnan(tz); %use the same/consistent mask as jump.tif
	    tzpre(M)=tz(M);
	    data0T2.z=tzpre;
	    ...
	    %add time of change T1
	    T2file=strrep(file1,'_jump.tif','_eventtimeT1.tif');
	    if ~exist(T2file,'file')
              fprintf(['\n file does not exist:',T2file,'. \n'])
	%     return
        end
	    dataT2=readGeotiff(T2file);
	    dataT2.z(dataT2.z<=20000101)=NaN;
	    tz=interp2(dataT2.x,dataT2.y,double(dataT2.z),datao.x,datao.y','*nearest',nan);
	    tzpre=datao.z(:,:,3);
% 	    M=isnan(tzpre)&~isnan(tz); %use the same/consistent mask as jump.tif
	    tzpre(M)=tz(M);
	    datao.z(:,:,3)=tzpre;
        ...
        %add jumpstd
	    T2file=strrep(file1,'_jump.tif','_jumpstd.tif');
	    if ~exist(T2file,'file')
              fprintf(['\n file does not exist:',T2file,'. \n'])
	%     return
        end
	    data1=readGeotiff(T2file);
	    data1.z(data1.z==0)=NaN;
	    tz=interp2(data1.x,data1.y,double(data1.z),datao.x,datao.y','*linear',nan);
	    tzpre=datao.z(:,:,2);
% 	    M=isnan(tzpre)&~isnan(tz); %use the same/consistent mask as jump.tif
	    tzpre(M)=tz(M);
	    datao.z(:,:,2)=tzpre;
        	    ...
        	    %add time of change T1
	    T2file=strrep(file1,'_jump.tif','_bitmask.tif');
	    if ~exist(T2file,'file')
              fprintf(['\n file does not exist:',T2file,'. \n'])
	%     return
        end
...
%    ot1file=['t1M',num2str(k1),'.mat'];
%    save(ot1file,'M','tz','tzpre','data','data0'); 
        
        	    %add time of change T1
	    T2file=strrep(file1,'_jump.tif','_nov.tif');
	    if ~exist(T2file,'file')
              fprintf(['\n file does not exist:',T2file,'. \n'])
	%     return
        end
	    dataT2=readGeotiff(T2file);
	    dataT2.z(dataT2.z<=0)=NaN;
	    tz=interp2(dataT2.x,dataT2.y,double(dataT2.z),datao.x,datao.y','*nearest',nan);
	    tzpre=datao.z(:,:,6);
% 	    M=isnan(tzpre)&~isnan(tz); %use the same/consistent mask as jump.tif
	    tzpre(M)=tz(M);
	...
	if 1 % flagband==1 
	   %datao=data0;
	   datao.z(:,:,1)=data0.z; %elevation change
...
%if ~exist(file1,'file')
	...
	fdir=strrep(fdir,'/home/chunli/chunliwork/ArcticDEMdata','/blue/chunlidai/data/ArcticDEMdata');
        ...
        oflag=0; %all strip data have been removed on server.
        if oflag==0
...
%         dem.x=data0.x;dem.y=data0.y;
    ...
    str1='mdf.txt';
        ...
        end % oflag %Done with mosaicking DEMs and Orthoimages.
