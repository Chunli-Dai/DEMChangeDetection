...
function [co,out]=dhelev_sub(jump,dem,M)
...
%     end
    ...
    elevdiff_step2=round(nanmin(elevchange)):0.2:round(nanmax(elevchange));
    ...
    %
    out.cri1=nanmedian(elevstep2(elevdiff_step2b<0)) - nanmedian(elevstep2(elevdiff_step2b>0));
    out.cri2=sum(abs(elevchange(elevchange<0))) - sum(elevchange(elevchange>0));
    out.cri3=cri3;
    
    %linear fit of the profile; y=b+kx;
    xobs=elevdiff_step2(1:end-1)';yobs=elevstep2';
    lenf=length(yobs);mp=2;
    ...
    out.slope=ksai(2);
    out.intercept=ksai(1);
    out.fitstd=sqrt(etilde'*etilde/(lenf-mp));
