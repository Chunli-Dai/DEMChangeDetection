...
% scenario 2: work on a negative elevation change cluster and the surrounding 100 m buffer zone;
% input: jump.mask if -1, the pixel is the target cluster; if 1, the pixe is the buffer zone, 0 means void.
...
featlabels={'slopeangle_mad','aspect_mad','Roughness_median','Roughness_mad','DCER1_median','DCER1_mad', ...
    ...
    'profc_median','profc_mad','planc_median','planc_mad','Kt_median','Kt_mad','totalc_median','totalc_mad', ...
    ...
    'dh_profc_median','dh_profc_mad','dh_planc_median','dh_planc_mad','dh_Kt_median','dh_Kt_mad','dh_totalc_median','dh_totalc_mad', ...
    'cri1','cri2','cri3','slope','intercept','fitstd'}; %cells.
m=length(featlabels);
...
feat=[dem_feat.slopeangle_mad dem_feat.slopeangle_mad dem_feat.aspect_mad dem_feat.Roughness_median dem_feat.Roughness_mad dem_feat.DCER1_median dem_feat.DCER1_mad  ...
...
if scenario == 2
    Mc=jump.mask==-1;
    [flagi,DEMdh2]=dhelev_sub(jump,DEM,Mc);
    feat(end-5:end)=[DEMdh2.cri1 DEMdh2.cri2 DEMdh2.cri3 DEMdh2.slope DEMdh2.intercept DEMdh2.fitstd];
end
...
out.TPI_median=median(TPI(:));
out.TPI_mad=mad(TPI(:),1);
