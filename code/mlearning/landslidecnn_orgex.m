...
stopSigns = stopSignsAndCars(:, {'imageFilename','carFront'});
stopSigns(6,:) = [];
stopSigns(8:33,:) = [];
stopSigns(9:end,:) = [];
...
I = insertObjectAnnotation(I,'Rectangle',stopSigns.carFront{1},'car Front','LineWidth',8);

else %stopSignMap is not consistent with outputImage
    stopSigns = stopSignsAndCars(:, {'imageFilename','carRear'});
stopSigns(5:6,:) = [];
stopSigns(8:17,:) = [];
stopSigns(8:12,:) = [];
stopSigns(9:22,:) = [];
...
% Display one training image and the ground truth bounding boxes
I = imread(stopSigns.imageFilename{1});
I = insertObjectAnnotation(I,'Rectangle',stopSigns.carRear{1},'car Rear','LineWidth',8);
...
figure;
imagesc(stopSignMap);title('classification scores')