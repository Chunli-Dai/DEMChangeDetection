...
function [yearv,tempe_yr_med]=latlon2tempe(latin,lonin,flagmethod,Soi)
%given lat lon, output temperature time series.
...
elseif flagdata==2
    %
    %2m temperature;
    ncFile='./era5/ERA5.nc'; %2008 to 2023, June July August September, hourly
    %Precipitation:2008 to 2023, April May June July August September, hourly
    flagmon=0; % 1, montlhy data; 0 hourly data
    
    if flagmethod==3
%     ncFile='./era5/ERA5tempGreenland2010to22hr.nc';
        
        if flagmon==1
%       ncFile='/Users/chunlidai/test/era5/monthly_temp.nc'; % lon 0 to 360
      ncFile='/Users/chunlidai/test/era5/monthly_temppreci.nc'; % lon 0 to 360
            if lonin<0; lonin=lonin+360;end
        elseif flagmon==0
      ncFile='/Users/chunlidai/test/era5/ERA5tempGreenland2009to13hr.nc';
%       ncFile='/Users/chunlidai/test/era5/ERA5tempGreenland2014to19hr.nc';
        end
    end
...
%     Mrts2=false(nlon,nlat);
    ...
    elseif flagmethod==3
         yearv=(timed)/365.25; %years from datenum %Original interval
...
%         Mrts2(ilon,ilat)=1;
        ...
        elseif flagmon==1 %if flagmethod==3 % monthly data
            % 1440 721 2 199        
            start  = [ilon ilat 1 1]; 
            count  = [1 1 1 Inf]; 
        ...
        tempe = ncread(ncFile, 't2m',start,count,stride);
        % Sep. 2024; scale and offset; already applied in ncread.m

%         if flagmethod ==3 % hourly
%             tempe_yr_med_n(:,i)=tempe(1,1,:); %hourly
%             continue
%         end
...
%                 Mt=floor(timed)==yearv(k)*365.25; % find all data within the selected day.
    ...
    tempe_yr_med=nanmean(tempe_yr_med_n,2) -273.15;
