...
elseif dataflag==4 % Both image are DEM files
    ...
    %get the image filenames
    OutName1= strrep(filename1,'_dem','_dem_prep.tif'); %filename1: no directory, no extension
    OutName1= [odir,OutName1];
    OutName2= strrep(filename2,'_dem','_dem_prep.tif'); %filename1: no directory, no extension
    OutName2= [odir,OutName2];

    if 1 %old
    data1i=readGeotiff(infile1,'map_subset',rang0);
    data2i=readGeotiff(infile2,'map_subset',rang0);
	...
	% 2\ prefer to keep the option of not apply bitmask.	
        metafile1=strrep(infile1,'dem.tif','meta.txt');
        if ~exist(metafile1,'file')
          metafile1=strrep(infile1,'dem.tif','mdf.txt');
	end
        [XYbi,~]=imagebd(metafile1);
        metafile2=strrep(infile2,'dem.tif','meta.txt');
        if ~exist(metafile2,'file')
          metafile2=strrep(infile2,'dem.tif','mdf.txt');
	end
        [XYbi2,~]=imagebd(metafile2);
	resr=2; %Hope SETSM works better on 2m rather than 40m.
