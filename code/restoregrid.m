...
M=double(idun==0);
%For the clusters of idun==0 (undesinated group),ignore the small clusters 
if 0% %old; Could undesirebaly remove everything if the total work area is only 1km by 1km 
    Modj=bwareaopen(M, nx2*nx2); %ignore pixel groups that have area < nx2*nx2 *dx*dx
else
    [n,m]=size(idun);
    pixellmt=min(nx2*nx2,n*m);
    Modj=bwareaopen(M,pixellmt);  
end
